...
# -----------------------------------------------------------
# Linkage allele/event encoding for indels
# -----------------------------------------------------------
def _encode_insertion_event(ins_seq, max_seq_len=None):
    """
    Encode insertion as an event token that is safe for downstream parsing.
    Format: I{len}_{seq}
    If max_seq_len is set and the sequence is longer, it may be truncated and marked with _TRUNC.
    Note: avoid ':', '-', ';', '(' and ')' because LinkageSource uses them as delimiters.
    """
    s = str(ins_seq).upper()
    L = len(s)
    if L == 0:
    ...
    if max_seq_len is not None and L > max_seq_len:
        return f"I{L}_{s[:max_seq_len]}_TRUNC"
    return f"I{L}_{s}"


def _encode_deletion_event(length):
    """Encode deletion as an event token: D{len}."""
    try:
        L = int(length)
    except Exception:
        L = 0
...
def _allele_event_meta(token):
    """
    Parse event token into (type, length, seq) for links CSV.
    - SNP base: A/C/G/T/N -> (SNP, 1, base)
    - Insertion: I{len}_{seq} -> (INS, len, seq)
    - Deletion: D{len} -> (DEL, len, '')
    """
    if token in ("A", "C", "G", "T", "N"):
        return ("SNP", 1, token)
    if token.startswith("I"):
        ...
        if i < len(body) and body[i] == "_":
            seq = body[i + 1 :]
        try:
            L = int(length_str) if length_str else ""
        except ValueError:
            L = ""
        return ("INS", L, seq)
...
def _format_event_counts(event_counts):
    """
    Format a dict {token: count} into a semicolon-separated string: token(count);token2(count2)
    """
    if not event_counts:
        return ""
    parts = []
    for token, c in sorted(event_counts.items(), key=lambda x: (-x[1], x[0])):
        try:
            n = int(c)
        except Exception:
            continue
...
def collect_info_alleles(cigar, ref, read, reference_pos, phred, profile={}):
    ...
    for b in break_points:
        ...
        if op == 'S':
            ...
            read_substr = read[read_pos:read_pos+length]
            # Anchor insertion to the base immediately before it (ref_pos-1).
            # If insertion happens before the first aligned base, we skip it to avoid negative coordinates.
            if ref_pos <= 0:
                read_pos += length
                continue
            ...
            # Track insertion event sequences (so downstream can reconstruct without relying on links)
            ins_token = _encode_insertion_event(read_substr)
            events = profile[reference_pos+ref_pos-1].setdefault('events', {})
            events.setdefault('I', {})
            events['I'][ins_token] = events['I'].get(ins_token, 0) + 1
            ...
            ref_substr = ref[ref_pos:ref_pos+length]
            if ref_substr:
                # Keep old icHET calling semantics: a multi-base deletion
                # contributes one D observation at each deleted reference base.
                # Separately, keep one event token anchored at the first deleted
                # base for downstream linkage/haplotype reconstruction.
                event_pos = reference_pos + ref_pos
                if event_pos not in profile:
                    profile[event_pos] = {'ref': ref_substr[0], 'err': {}}
                    profile[event_pos]['err'][ref_substr[0]] = []
                del_token = _encode_deletion_event(length)
                events = profile[event_pos].setdefault('events', {})
                events.setdefault('D', {})
                events['D'][del_token] = events['D'].get(del_token, 0) + 1

                for i in range(length):
                    cur_pos = reference_pos + ref_pos
                    if cur_pos not in profile:
                        profile[cur_pos] = {'ref': ref_substr[i], 'err': {}}
                        profile[cur_pos]['err'][ref_substr[i]] = []
                    profile[cur_pos]['err'].setdefault('D', [])
                    profile[cur_pos]['err']['D'].append(0)
                    ref_pos += 1
            else:
                ref_pos += length
...
def analyze_positions(profile, ANNOTATION_FILE, out_csv):
    ...
    for pos, p in sorted(profile.items()):
        ...
        position_data = {
            ...
            'D': len(p['err'].get('D', [])),
            'I': len(p['err'].get('I', [])),
            'DEvents': _format_event_counts(events.get('D', {})),
            'IEvents': _format_event_counts(events.get('I', {})),
            'Total': count,
            'Percentage': minor_f,
    ...
    with open(out_csv, 'w') as f:
        ...
        header = [
            "Pos", "Ref", "A", "C", "G", "T", "D", "I", "DEvents", "IEvents", "Total", 
        ...
        for res in results:
            ...
            row_data = [
                ...
                res['D'], res['I'], res['DEvents'], res['IEvents'], res['Total'], res['Percentage'], res['Ea'], 
...
def parse_sam_file(filename, ref_seq, annotation_file, out_csv, out_links=None):
    profile = {}
    count = 0
    adj = {}
    ...
    with open(filename, "rU") as file:
        for line in file:
            if line[0] != '@':
                ...
                if out_links:
                    # collect linkage: variant positions and alleles in this read
                    var_alleles = {}  # pos -> set of allele/event tokens observed on this read
                    break_points = [ i[0] for i in enumerate(cigar) if i[1].isalpha() ]
                    start = 0
                    read_pos = 0
                    ref_offset = 0
                    for b in break_points:
                        ...
                        if op == 'S':
                            ...
                            ref_substr = seq[ref_offset:ref_offset+length]
                            read_substr = read[read_pos:read_pos+length]
                            for i, base in enumerate(read_substr):
                                if base != ref_substr[i]:
                                    pos_key = ref_pos + ref_offset + i
                                    var_alleles.setdefault(pos_key, set()).add(base)
                            read_pos += length
                            ref_offset += length
                        elif op == 'I':
                            # insertion assigned to the position before the insertion (ref_offset-1)
                            ins_seq = read[read_pos:read_pos+length]
                            ins_pos = ref_pos + ref_offset - 1
                            if ins_pos >= 0:
                                var_alleles.setdefault(ins_pos, set()).add(_encode_insertion_event(ins_seq))
                            read_pos += length
                        elif op == 'D':
                            # deletion represented as a single event anchored at the first deleted base
                            del_pos = ref_pos + ref_offset
                            if del_pos >= 0:
                                var_alleles.setdefault(del_pos, set()).add(_encode_deletion_event(length))
                            ref_offset += length
                        elif op == 'H':
                            pass
                    if len(var_alleles) >= 2:
                        for a, b in itertools.combinations(sorted(var_alleles.keys()), 2):
                            for allele1 in var_alleles[a]:
                                for allele2 in var_alleles[b]:
                                    allele_pair = (allele1, allele2)
                                    adj.setdefault((a, b), {})
                                    adj[(a, b)].setdefault(allele_pair, 0)
                                    adj[(a, b)][allele_pair] += 1

    # Pre-compute overlap denominators for each (pos1,pos2) edge:
    # n_overlap = number of reads whose reference span covers both positions.
    # This provides the "opportunity count" denominator for strict edge-level statistics downstream.
    overlap = {}
    edge_positions = []
    pos_partners = {}
    if out_links and adj:
        overlap = {}
        pos_partners = {}
        for (a, b) in adj.keys():
            pos_partners.setdefault(a, set()).add(b)
            pos_partners.setdefault(b, set()).add(a)
        edge_positions = sorted(pos_partners.keys())
    ...
    with open(filename, "rU") as file:
        for line in file:
            if line[0] != '@':
                ...
                if out_links and edge_positions:
                    # reference span for this read (0-based, end-exclusive)
                    start = ref_pos
                    end = ref_pos + len(seq)
                    if end - start >= 2:
                        L = bisect.bisect_left(edge_positions, start)
                        R = bisect.bisect_left(edge_positions, end)
                        covered = edge_positions[L:R]
                        if len(covered) >= 2:
                            covered_set = set(covered)
                            for a in covered:
                                for b in pos_partners.get(a, ()):
                                    if b in covered_set and a < b:
                                        overlap[(a, b)] = overlap.get((a, b), 0) + 1

    analyze_positions(profile, annotation_file, out_csv)
    if out_links:
        with open(out_links, 'w') as f:
            f.write('pos1,pos2,allele1,allele2,count,n_overlap,allele1_type,allele1_len,allele1_seq,allele2_type,allele2_len,allele2_seq\n')
            for (a, b), allele_counts in sorted(adj.items()):
                n_overlap = overlap.get((a, b), 0) if overlap else 0
                for (allele1, allele2), c in sorted(allele_counts.items(), key=lambda x: -x[1]):
                    t1, l1, s1 = _allele_event_meta(allele1)
                    t2, l2, s2 = _allele_event_meta(allele2)
                    f.write(f"{a+1},{b+1},{allele1},{allele2},{c},{n_overlap},{t1},{l1},{s1},{t2},{l2},{s2}\n")
...
def process(**params):
    ...
    out_links = params.get('out_links')
    parse_sam_file(alignment_output, record.seq, annotation_file, out_csv, out_links)


if __name__ == '__main__':
    if len(sys.argv) not in (5,6):
        print("Usage:", sys.argv[0] + "  reference.fasta  alignment_output.sam  annotation_file output_csv [out_links]")
    else:
        ...
        params = {
            ...
            'out_csv' : sys.argv[4],
            'out_links' : sys.argv[5] if len(sys.argv)==6 else None,
        }
