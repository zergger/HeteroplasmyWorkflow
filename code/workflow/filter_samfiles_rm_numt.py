...
def filter_reads_for_cp_mt(filtered_sam, SAMdir, chloroplast, mitochondria):
    """
    Filters a SAM file for chloroplast or mitochondria reads, 
    preserving the SAM header in the output files.
    """
    genome_name = {}
    ...
    if len(genome_name) > 0:
        ...
        with open(filtered_sam, "r") as file:
            for line in file:
                # [FIX] First, check if it's a header line
                if line.startswith('@'):
                    # Write the header line to all active output files
                    if chloroplast != 'None':
                        cpf.write(line)
                    if mitochondria != 'None':
                        mtf.write(line)
